...
small as _"does the binary spawn?"_ — seconds with `is-odd`. About two hours bought six booleans.
...
| 2   | same, with the config/override env set          | the feature is compiled in AND the override _engages_   |
...
- **Do not read "Landlock unavailable" as a kernel problem.** That error covers both an old kernel
  and _our own policy failing to compile_; it claimed a missing feature on a 6.17 kernel with ABI 4.
