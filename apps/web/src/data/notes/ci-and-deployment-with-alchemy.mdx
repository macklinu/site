...
- Keep `nub run --node build` and `nub run --node astro dev` aligned with the app's normal build and dev commands.
...
jobs:
  deploy-preview:
    ...
    steps:
      # repeat the checkout, setup-nub, and install steps from verify.yml
